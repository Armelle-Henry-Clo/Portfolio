<!DOCTYPE html>
<html lang="fr">
<head>
    <meta charset="UTF-8">
    <meta http-equiv="X-UA-Compatible" content="IE=edge">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <!-- FAVICON -->
    <link
      rel="icon"
      type="image/png"
      href="https://img.icons8.com/color/2x/beach.png">

    <title>Fermez les yeux!</title>
    <!-----Bootstrap----->
    <link href="https://cdn.jsdelivr.net/npm/bootstrap@5.1.3/dist/css/bootstrap.min.css" rel="stylesheet" integrity="sha384-1BmE4kWBq78iYhFldvKuhfTAU6auU8tT94WrHftjDbrCEXSU1oBoqyl2QvZ6jIW3" crossorigin="anonymous">
<!-----------------Mon CSS--------------------->
    <link rel="stylesheet" href="css/style.css">
</head>
       
<body>    
    
        <div class="container mx-auto">
            
            <h1 class="text-center text-light ">Fermez les yeux!</h1> 
            <!-------Video------>
            <div class="row">
                <iframe class ="mx-auto" src="https://www.youtube.com/embed/dZiLEgngU4o" height="300px" title="YouTube video player" frameborder="0" allow="accelerometer; autoplay; encrypted-media; gyroscope;" allowfullscreen></iframe>
            </div>

            <div class="row">
                <iframe class ="mx-auto justify-content-md-center" src="https://www.youtube.com/embed/yzglKj_v2O4" height="300px" title="YouTube video player" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture" allowfullscreen></iframe>
            </div>
            
            <div class="row">
                <iframe class ="mx-auto" src="https://www.youtube.com/embed/8ORQ6SoLIvU" height="300px" title="YouTube video player" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture" allowfullscreen></iframe>
            </div>
        </div>

     <!-------------------Footer--------------->
     <footer>
        <div class="d-flex justify-content-around pb-3 pt-5 " style="height: 150px">
          <div class="col-3 text-center">
             <a href="index_p2.php" target="blank" class="link-dark "> Nous contacter </a>
<!------------- icones reseaux sociaux--------------------------->  
            <div>
              <a href="https://fr-fr.facebook.com/"><img src="data:image/png;base64," alt="facebook"/></a>
              <a href="https://www.instagram.com/?hl=fr"><img src="data:image/png;base64," alt="instagram"/></a>
              <a target="_blank" href="https://www.tiktok.com/fr/"><img src="data:image/png;base64," alt="tiktok"/></a>

            </div>
          </div>
                      
          <div class="col-3 text-center">
            <a href="index_p1.php" target="blank" class="link-dark "> Accueil </a>
          </div>
 <!-----------------Liens vers iframe------------------------------->
          <div class="col-3 text-center">
            <p>Nos partenaires</p>
            <a href="https://www.airbnb.fr/" target="blank"><img src="data:image/png;base64,"/></a>
            <a href="https://www.tripadvisor.fr/"><img src="data:image/png;base64,"/></a>
          </div>
        
        </div>
      </footer>

    <script src="https://cdn.jsdelivr.net/npm/@popperjs/core@2.10.2/dist/umd/popper.min.js" integrity="sha384-7+zCNj/IqJ95wo16oMtfsKbZ9ccEh31eOz1HGyDuCQ6wgnyJNSYdrPa03rtR1zdB" crossorigin="anonymous"></script>
    <script src="https://cdn.jsdelivr.net/npm/bootstrap@5.1.3/dist/js/bootstrap.min.js" integrity="sha384-QJHtvGhmr9XOIpI6YVutG+2QOK9T+ZnN4kzFN1RtK3zEFEIsxhlmWl5/YESvpZ13" crossorigin="anonymous"></script>
    </body>
</html>